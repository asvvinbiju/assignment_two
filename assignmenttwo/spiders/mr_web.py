...
class MrWeb(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        chrome_options.binary_location = (
            "/home/asvvinbiju/chrome/linux-114.0.5735.90/chrome-linux64/chrome"
        )
        chrome_options.headless = False
        self.driver = webdriver.Chrome(
            service=ChromeService("/usr/local/bin/chromedriver"), options=chrome_options
        )
    ...
    def parse(self, response):
        ...
        while len(self.properties_url) <= 200:
            wait.until(
                EC.presence_of_element_located((By.XPATH, '//span[@class="_2NPFF"]'))
            )
            while True:
                try:
                    click_button = self.driver.find_element(
                        By.CSS_SELECTOR, '[class="rui-apowA rui-htytx rui-UGVY0"]'
                    )
                    if click_button:
                        self.driver.execute_script(
                            "arguments[0].scrollIntoView(true);", click_button
                        )
                except:
                    continue
                else:
            ...
            try:
                wait.until(
                    EC.visibility_of_element_located(
                        (By.XPATH, '//figure[@class="_3UrC5"]/img')
                    )
                )
            except:
                pass
    ...
    def parse_properties(self, response):
        ...
        price = response.xpath(
            '//section[@class="_8S0h4"]/span[@class="T8y-z"]/text()'
        ).get()
        currency = ""
        proprety_price = None
        if price:
        ...
        detail_ = details.xpath('//span[@class="B6X7c"]/text()').getall()
        ...
        yield {
            ...
            "description": response.xpath(
                '//div[@class="rui-oN78c ok1RR"]/div/p/text()'
            ).get(),
            "seller_name": response.xpath('//div[@class="_1ibEV"]/div/a/@title').get(),
            "location": response.xpath(
                '//div[@class="rui-oN78c"]/div/span/text()'
            ).get(),
            "property_type": type,
            "bathroom": bathrooms,
            "bedroom": bedrooms,
